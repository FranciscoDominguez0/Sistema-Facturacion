...
<a href="{{ $href }}" wire:navigate @click="currentPath = new URL($el.href).pathname; abierto = false"
    :class="{{ $objetoClasesEnlace }}" :aria-current="{{ $expresionActiva }} ? 'page' : null"
    ...
    <div class="w-8 h-8 shrink-0 rounded-full flex items-center justify-center transition-all {{ $clasesIcono }}"
        ...
        <span class="material-symbols-outlined text-[20px]"
            :style="{{ $expresionActiva }} ? 'font-variation-settings: \'FILL\' 1;' : ''" @if ($activo)
            style="{{ $estiloIcono }}" @endif>{{ $icono }}</span>
    </div>
    <span class="text-sm tracking-wide {{ $pesoEtiqueta }}" :class="{{ $objetoPesoEtiqueta }}">{{ $slot }}</span>
</a>