        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="flex flex-col gap-5">
                <div class="flex flex-col gap-1.5">
                    ...
                    <input wire:model.live="form.prefijo_factura" class="bg-slate-50 border border-slate-300 rounded-lg px-4 py-2 text-sm text-slate-900 focus:outline-none focus:border-sovereign-blue focus:ring-1 focus:ring-sovereign-blue transition-all" type="text" placeholder="FAC-"/>
                    <span class="text-xs text-slate-400">Caracteres que preceden al número. Ej: FAC-, INV-, 2024-</span>
                    @error('form.prefijo_factura') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-bold uppercase tracking-wider text-slate-500">Siguiente Número</label>
                    <input wire:model.live="form.siguiente_numero_factura" class="bg-slate-50 border border-slate-300 rounded-lg px-4 py-2 text-sm text-slate-900 focus:outline-none focus:border-sovereign-blue focus:ring-1 focus:ring-sovereign-blue transition-all" type="number" min="1"/>
                    <span class="text-xs text-slate-400">Se incrementará automáticamente.</span>
                    @error('form.siguiente_numero_factura') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>
            </div>
            <div class="flex flex-col gap-2">
                ...
                <div class="bg-slate-50 rounded-lg border border-slate-200 p-4 flex flex-col gap-3">
                    <div class="flex justify-between items-center bg-white p-3 rounded-lg border border-slate-200 opacity-60">
                        ...
                        <span class="font-medium text-slate-900">{{ $this->numero_preview_anterior }}</span>
                    </div>
                    ...
                    <div class="flex justify-between items-center bg-white p-3 rounded-lg border border-slate-200 opacity-60">
                        ...
                        <span class="font-medium text-slate-900">{{ $this->numero_preview_siguiente }}</span>
                    </div>
                </div>
            </div>
        </div>
